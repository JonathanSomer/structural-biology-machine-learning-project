...
if ($#ARGV != 3 and $#ARGV != 2 and $#ARGV != 1) {
  print "buildParamsFine.pl <receptorPDB> <ligandPDB> [rmsd default=2.0] [moleculesTypes]\n";
...
if($molType =~ /drug/) {
  ...
  if($molType eq "EI") {
    print OUT "receptorSeg 10.0 20.0 1.5 1 1 1 0\n";
    print OUT "ligandSeg 10.0 20.0 1.0 1 1 1 0\n";
  } else {
    if($molType eq "AA") {
      print OUT "receptorSeg 10.0 20.0 1.5 1 1 1 2\n";
      print OUT "ligandSeg 10.0 20.0 1.0 1 1 1 1\n";
    } else {
      if(countAtoms($recPdb) > 2000 and countAtoms($ligPdb) > 2000) {
	print OUT "receptorSeg 10.0 20.0 2.0 1 1 1 0\n";
	print OUT "ligandSeg 10.0 20.0 1.5 1 1 1 0\n";
      } else {
	print OUT "receptorSeg 10.0 20.0 1.5 1 1 1 0\n";
	print OUT "ligandSeg 10.0 20.0 1.0 1 1 1 0\n";
      }
...
print OUT "# do not perform full final scoring - if the score is not going to be used later for ranking, but some other criteria\n";
print OUT "fullFinalScoring 0\n";
...
if( $molType =~ /drug/) {
  ...
  if($molType eq "AA") {
    if( $rmsd == -1.0) {
      print OUT "clusterParams 0.1 3 2.0 2.0\n";
    } else {
      print OUT "clusterParams 0.1 3 2.0 $rmsd\n";
    }
  } else {
    if( $rmsd == -1.0) {
      print OUT "clusterParams 0.1 4 2.0 2.0\n";
    } else {
